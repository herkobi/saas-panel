...
class SubscriptionRenewedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ...
            ->line($planName . ' planı aboneliğiniz başarıyla yenilendi.')
            ->line('Abonelik detayları:')
            ->line('Plan: ' . $planName)
            ->line('Dönem: ' . $billingPeriod)
            ->line('Yeni Bitiş Tarihi: ' . $endDate)
            ->action('Abonelik Bilgilerini Görüntüle', route('app.subscription.plans'))
            ->line('Hizmetimizi kullanmaya devam ettiğiniz için teşekkür ederiz!');
    }
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'message' => $this->subscription->plan->name . ' planı aboneliğiniz başarıyla yenilendi.',
        ];
    }
}
