...
<template>
    <AppLayout>
        <Head :title="`${space.name} Düzenle`" />

        <LinksLayout>
            <div class="space-y-6">
                <div class="flex items-center gap-2">
                    <Button variant="outline" size="icon" as-child>
                        <a :href="route('app.spaces')">
                            <ArrowLeft class="h-4 w-4" />
                        </a>
                    </Button>
                    <HeadingSmall title="Alan Düzenle" description="Alan bilgilerini bu form yardımıyla düzenleyebilirsiniz" />
                </div>

                <SpaceForm :space="space" :is-editing="true" />
            </div>
        </LinksLayout>
    </AppLayout>
</template>
