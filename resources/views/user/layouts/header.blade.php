<header
    ...
    <nav class="flex basis-full items-center w-full mx-auto px-4 sm:px-6 md:px-8" aria-label="{{ __('global.app') }}">
        <div class="me-5 lg:me-0 lg:hidden">
            <a class="flex-none text-xl font-semibold dark:text-white" href="#"
                aria-label="{{ __('global.app') }}">{{ __('global.app') }}</a>
        </div>
        <div class="w-full flex items-center justify-end ms-auto sm:justify-between sm:gap-x-3 sm:order-3">
            ...
            <div class="hidden sm:block">
                <label for="icon" class="sr-only">{{ __('user/dashboard/dashboard.search') }}</label>
                <div class="relative">
                    ...
                    <input type="text" id="icon" name="icon"
                        ...
                        placeholder="{{ __('user/dashboard/dashboard.search') }}">
                </div>
            </div>
                    ...
                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700"
                        ...
                        <div class="py-3 px-5 -m-2 bg-gray-100 rounded-t-lg dark:bg-gray-700">
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                {{ __('user/dashboard/dashboard.logged') }}</p>
                        ...
                        <div class="mt-2 py-2 first:pt-0 last:pb-0">
                            <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300"
                                ...
                                {{ __('user/dashboard/dashboard.profile') }}
                            </a>
                            <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300"
                                href="#">
                                <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24"
                                ...
                                {{ __('user/dashboard/dashboard.account') }}
                            </a>
                            <form method="POST" action="{{ route('app.logout') }}">
                                ...
                                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300"
                                    ...
                                    {{ __('user/dashboard/dashboard.logout') }}
                                </a>
                            </form>
                        </div>
                    </div>
