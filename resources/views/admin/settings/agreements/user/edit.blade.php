    ...
    <div class="page-content">
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-9">
                    <h3 class="form-title border-bottom mb-3 pb-3">Sözleşme Bilgilerini Düzenle</h3>
                    <form action="{{ route('panel.settings.agreement.user.update', $agreement->id) }}" method="POST">
                        @csrf
                                    ...
                                    @foreach (Status::cases() as $type)
                                        <label class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status"
                                                value="{{ $type->value }}"
                                                {{ $agreement->status == $type->value ? 'checked' : '' }}>
                                ...
                                <div>
                                    <label class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="show_on_register"
                                            value="1"
                                            {{ old('show_on_register', $agreement->show_on_register) === true ? 'checked' : '' }}>
                                        <span class="form-check-label">EVET</span>
                                    </label>
                                    <label class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="show_on_register"
                                            value="0"
                                            {{ old('show_on_register', $agreement->show_on_register) === false ? 'checked' : '' }}>
                                ...
                                <div>
                                    <label class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="show_on_payment" value="1"
                                            {{ old('show_on_payment', $agreement->show_on_payment) === true ? 'checked' : '' }}>
                                        <span class="form-check-label">EVET</span>
                                    </label>
                                    <label class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="show_on_payment" value="0"
                                            {{ old('show_on_payment', $agreement->show_on_payment) === false ? 'checked' : '' }}>
                                        <span class="form-check-label">HAYIR</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-9 col-md-8">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="description" id="description"
                                        ...
                                        placeholder="Sözleşme Açıklaması Giriniz"
                                        value="{{ old('description', $agreement->description) }}">
                                </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('panel.settings.agreement.user.delete', $agreement->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="modal-body">
                        <strong>{{ $agreement->title }}</strong> isimli sözleşmeyi silmek üzeresiniz. Bu işlem geri
                    ...
                    <div class="modal-footer">
                        <div class="d-flex align-items-center justify-content-between w-100">
                            <button type="button" class="btn bg-white border-0 text-dark" data-bs-dismiss="modal">İptal
                                Et</button>
                            <button type="submit" class="btn">Evet, Sözleşmeyi Sil</button>
                        </div>
                    </div>
                </form>
            </div>
