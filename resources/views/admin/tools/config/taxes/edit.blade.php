    ...
    <div class="page-content">
        <div class="container">
            <div class="row mb-5">
                ...
                <div class="col-lg-9">
                    <div class="page-form">
                        <h3 class="form-title border-bottom mb-3 pb-3">Vergi Oranı Bilgilerini Düzenle</h3>
                        <form action="{{ route('panel.tools.config.tax.update', $tax->id) }}" method="POST">
                            @csrf
                                        ...
                                        @foreach (Status::cases() as $type)
                                            <label class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status"
                                                    value="{{ $type->value }}"
                                                    {{ $type->value == $tax->status->value ? 'checked' : '' }}>
                                ...
                                <div class="col-lg-10 col-md-9">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="title" id="title"
                                            ...
                                            placeholder="Vergi Oranı Adını Giriniz" value="{{ old('title', $tax->title) }}"
                                            required>
                                    </div>
                                        ...
                                        <div class="col-lg-6 mb-1">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input id="value" type="text" name="value"
                                                    class="form-control rounded-0 border-start-0"
                                                    placeholder="Vergi Oranı Değerini Giriniz"
                                    ...
                                    <div class="row mb-1">
                                            ...
                                            <select class="form-select rounded-0 tom-select" name="regions[0][country_id]"
                                                ...
                                                @foreach ($countries as $countryItem)
                                                    <option value="{{ $countryItem->id }}"
                                                        {{ (old('regions.0.country_id') ?? $tax->regions->first()?->country_id) == $countryItem->id ? 'selected' : '' }}>
                                                        {{ $countryItem->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-6 mb-1">
                                            <select class="form-select rounded-0 tom-select" name="regions[0][state_id]"
                                                id="state_id"
                                                data-current-state="{{ $tax->regions->first()?->state_id }}">
                                                @if ($tax->regions->first()?->state)
                                                    <option value="{{ $tax->regions->first()->state->id }}" selected>
                                                        {{ $tax->regions->first()->state->name }}
                                                    </option>
                                                @endif
                                            </select>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('panel.tools.config.tax.delete', $tax->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="modal-body">
                        <strong>{{ $tax->title . ' ' . $tax->code . ' - %' . $tax->value }}</strong> isimli vergi oranını
                        silmek üzeresiniz. Bu işlem geri alınamaz. Emin misiniz?
                    </div>
                    <div class="modal-footer">
                        <div class="d-flex align-items-center justify-content-between w-100">
                            <button type="button" class="btn bg-white border-0 text-dark" data-bs-dismiss="modal">İptal
                                Et</button>
                            <button type="submit" class="btn">Evet, Vergi Oranını Sil</button>
                        </div>
                    </div>
                </form>
            </div>
